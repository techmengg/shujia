...
const USER_AGENT =
  "Shujia/0.1 (+https://github.com/techmengg/shujia; contact@localhost)";
...
export interface MangaUpdatesRequestInit extends Omit<RequestInit, "body"> {
  body?: unknown;
...
function parseRetryAfter(headerValue: string | null, fallbackMs: number): number {
  ...
  if (Number.isFinite(dateTarget)) {
    ...
    if (diff > 0) return Math.max(fallbackMs, diff);
  }
  return fallbackMs;
}
...
export async function mangaupdatesFetch<TResponse>(
  ...
  const { body, headers, retry, method, ...rest } = init;
  ...
  while (attempt < attempts) {
    ...
    try {
      const data = (await response.json()) as { reason?: string; context?: string };
      message = data.reason ?? data.context ?? message;
    } catch {
