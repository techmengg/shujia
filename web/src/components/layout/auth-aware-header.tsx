...
/**
 * Client-side auth-aware header that handles hydration properly
 * Checks cookies on mount to sync with actual auth state
 */
export function AuthAwareHeader({ logoSrc, initialUser, theme }: AuthAwareHeaderProps) {
  const [user, setUser] = useState(initialUser);
  const [mounted, setMounted] = useState(false);

  // Check actual auth state on client
  useEffect(() => {
    setMounted(true);
  ...
  return (
    <header className="relative z-10 border-b border-white/15 bg-black">
      <div className="mx-auto flex w-full max-w-7xl flex-nowrap items-center gap-2 px-4 py-2 sm:px-6 lg:px-10 lg:py-3">
        ...
        <div className="order-2 flex w-full items-center gap-3 md:order-2 md:flex-1">
          ...
          <div className="min-w-0 flex-1">
            <SearchBar isAuthenticated={Boolean(currentUser)} />
          </div>
        </div>

        <div className="order-3 ml-auto flex shrink-0 items-center gap-2 text-surface-subtle">
          <MobileQuickActions
            settingsHref={currentUser ? "/settings/profile" : "/login?redirect=/settings/profile"}
